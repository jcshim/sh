...
class Model(object):
    ...
    # def infer_tgt_views(self, raw_src_image, raw_tgt_depth, RT, intrinsic):
    ...
    def into_depth_and_rgb_block(self, raw_src_image, raw_src_depth, pose):
        ...
        with tf.name_scope('Encoder'):
            z_enc_out = Encoder(input_rgbd, num_outputs=z_size, reuse_weights=tf.AUTO_REUSE)
            _, z_h, z_w, _ = z_enc_out.get_shape().as_list()
            # print('encoder out', z_enc_out)
        ...
        with tf.name_scope('Depth'):
            ...
            depth_dec_out = Decoder(z_geo_tf, 1, variable_scope='Depth_Decoder', reuse_weights=tf.AUTO_REUSE)
            depth_pred = depth_scale * tf.nn.tanh(depth_dec_out) + depth_bias

        with tf.name_scope('Pixel'):
            pixel_dec_out = Decoder(z_tf, 3, variable_scope='Pixel_Decoder', reuse_weights=tf.AUTO_REUSE)
            pixel_pred = tf.nn.tanh(pixel_dec_out)
            # print('pixel pred', pixel_pred)
    ...
    def build_train_graph(self, inputs):
        ...
        with tf.name_scope('inference'):
            ...
            with tf.name_scope('tgt2src'):
                ...
                predictions2 = self.into_depth_and_rgb_block(input_pred_tgt_image, pred_tgt_depth, inv_RT)
                pred_src_image = predictions2['out_pixel']
                pred_src_depth = predictions2['out_depth']

        with tf.name_scope('loss'):
            if self.data =='car':
                fg_src_image = tf.cast(raw_src_image < 255, dtype=tf.float32)
                fg_src_mask = fg_src_image[:,:,:,0]
            src_image = self.image2tensor(raw_src_image)
            src_depth = tf.expand_dims(raw_src_depth, axis=3)
            ...
            # loss 1
            depth_loss = helpers.masked_L1_Loss(pred_src_depth, src_depth, mask=fg_src_image)
            pixel_loss = helpers.masked_L1_Loss(pred_src_image, src_image, mask=fg_src_image)
            ...
            photometric_loss2 = self.compute_photometric_loss(pred_src_depth, pred_tgt_image, src_image,
                                                                 intrinsic, inv_RT, mask=fg_src_image)

            total_loss = depth_loss + pixel_loss + photometric_loss1 + photometric_loss2
